...
#define MAX_MARCHING_STEPS 1024
...
vec3 Render(in Ray ray) {
    ...
    if (info.Hit) {
        float shadow = 0.f;
        if (ShadowsEnabled) {
            shadow = 1.f-SoftShadow(Ray(info.Position + info.Normal * EPSILON, normalize(vec3(1))), 8);
        }
        
        vec3 ret = vec3(max(dot(info.Normal, normalize(vec3(1))), 0.f));
